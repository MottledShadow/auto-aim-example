...
TEST(CrcCalc, KnownAnswerVector) {
    ...
    EXPECT_EQ(auto_aim::crc::calc(data, sizeof(data)), 0x6F91);
}
...
TEST(CrcRoundTrip, AppendThenVerify) {
    ...
    auto_aim::crc::append(frame.data(), frame.size());
    EXPECT_TRUE(auto_aim::crc::verify(frame.data(), frame.size()));
}
...
TEST(CrcRoundTrip, AppendIsLittleEndian) {
    ...
    const std::uint16_t expected = auto_aim::crc::calc(frame.data(), frame.size() - 2);
    auto_aim::crc::append(frame.data(), frame.size());
...
TEST(CrcVerify, TamperedPayloadFails) {
    ...
    EXPECT_FALSE(auto_aim::crc::verify(frame.data(), frame.size()));
}
...
TEST(CrcVerify, DegenerateInputRejected) {
    EXPECT_FALSE(auto_aim::crc::verify(nullptr, 19));
    const std::uint8_t two[] = {0x12, 0x34};
    EXPECT_FALSE(auto_aim::crc::verify(two, sizeof(two)));   // 长度刚好等于 CRC 大小
    EXPECT_FALSE(auto_aim::crc::verify(two, 0));
}
...
TEST(CrcFrame, QuaternionFrameRoundTrip) {
    ...
    auto_aim::crc::append(frame.data(), frame.size());   // CRC 覆盖帧头+负载，写入 frame[17..18]
    EXPECT_TRUE(auto_aim::crc::verify(frame.data(), frame.size()));
}
...
TEST(CrcFrame, TargetFrameRoundTrip) {
    ...
    auto_aim::crc::append(frame.data(), frame.size());   // CRC 覆盖 frame[0..13]，写入 frame[14..15]
    EXPECT_TRUE(auto_aim::crc::verify(frame.data(), frame.size()));
}
