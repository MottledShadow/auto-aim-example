...
binary="$build_dir/preprocess_offline_test"

echo "Configuring ARM64 build..."
cmake -S "$project_dir" -B "$build_dir" \
    -DCMAKE_TOOLCHAIN_FILE="$project_dir/jetson-toolchain.cmake"

echo "Building preprocess offline test..."
cmake --build "$build_dir" --target preprocess_offline_test --parallel

echo "Deploying binary..."
# 复用 snapshot 的目录，captures/ 里就是刚拍下的照片
ssh "$target" 'mkdir -p "$HOME/cross-snapshot"'
scp "$binary" "$target:cross-snapshot/preprocess_offline_test"

echo "Running preprocess offline test on Jetson..."
ssh -t "$target" '
set -e
cd "$HOME/cross-snapshot"
chmod u+x preprocess_offline_test
# 读 captures/ 的照片，结果输出到 test_output/test_<时间戳>/
./preprocess_offline_test captures test_output
'
