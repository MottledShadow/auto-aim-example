...
# 用法：cross-run.sh <preprocess|lightbar|armor|pnp> <offline|online>
stage="${1:-}"
mode="${2:-}"
case "$stage" in
    preprocess|lightbar|armor|pnp) ;;
    *) echo "usage: $0 <preprocess|lightbar|armor|pnp> <offline|online>"; exit 1 ;;
esac
case "$mode" in
    offline|online) ;;
    *) echo "usage: $0 <preprocess|lightbar|armor|pnp> <offline|online>"; exit 1 ;;
