...
binary="$build_dir/armor_online_test"

echo "Configuring ARM64 build..."
cmake -S "$project_dir" -B "$build_dir" \
    -DCMAKE_TOOLCHAIN_FILE="$project_dir/jetson-toolchain.cmake"

echo "Building armor online test..."
cmake --build "$build_dir" --target armor_online_test --parallel

echo "Deploying binary..."
ssh "$target" 'mkdir -p "$HOME/cross-snapshot"'
scp "$binary" "$target:cross-snapshot/armor_online_test"

echo "Running armor online test on Jetson..."
# 实时窗口，需要 DISPLAY；q/ESC 退出
ssh -t "$target" '
set -e
cd "$HOME/cross-snapshot"
chmod u+x armor_online_test
